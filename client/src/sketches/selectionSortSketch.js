...
/**
 * p5.js sketch for visualizing selection sort.
 */
export const selectionSortSketch = (p) => {
  let values = [];
  let sorter = null;
  let isSorting = false;
  let speed = 5;
  ...
  p.setup = () => {
    p.createCanvas(cw, ch); // Increased height to accommodate index labels
    p.noLoop();
  ...
  p.draw = () => {
    ...
    if (isSorting && sorter) {
      if (!sorter.isSorted()) {
        ...
        const compareA = indices[1] !== undefined ? indices[1] : indices[0];
    ...
    const indices = sorter && sorter.getCurrentIndices ? sorter.getCurrentIndices() : [-1, -1, -1];
    const [i, j, minIdx] = indices;
    ...
    if (arr.length > 0) {
      for (let idx = 0; idx < arr.length; idx++) {
        ...
        if (sortedIndices.includes(idx) || (sorter && sorter.isSorted())) {
          ...
          p.fill(59, 130, 246); // blue for min
        } else if (idx === i || idx === j) {
          p.fill(239, 68, 68);
        } else {
          p.fill(55, 65, 81);
