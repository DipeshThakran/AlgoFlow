  ...
  p.draw = () => {
    ...
    if (isSorting && sorter) {
      if (!sorter.isSorted()) {
        ...
        const compareA = indices[0] >= 0 ? indices[0] : 0;
    ...
    const indices = sorter && sorter.getCurrentIndices ? sorter.getCurrentIndices() : [-1];
    ...
    if (arr.length > 0) {
      for (let idx = 0; idx < arr.length; idx++) {
        ...
        p.noStroke();
        p.rect(idx * barWidth + padding / 2, (p.height - (showLabels ? 30 : 10)) - barHeight, barWidth - padding, barHeight, 6);
      }
    }
