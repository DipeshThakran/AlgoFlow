...
/**
 * p5.js sketch for visualizing merge sort.
 */
export const mergeSortSketch = (p) => {
  let values = [];
  let sorter = null;
  let isSorting = false;
  ...
  p.draw = () => {
    ...
    const indices = sorter && sorter.getCurrentIndices ? sorter.getCurrentIndices() : [-1, -1];
    const [a, b] = indices;
    ...
    if (arr.length > 0) {
      for (let idx = 0; idx < arr.length; idx++) {
        ...
        if (sortedIndices.includes(idx) || (sorter && sorter.isSorted())) {
          p.fill(34, 197, 94); // green
        } else if (idx === a || idx === b) {
          p.fill(239, 68, 68); // red
        } else {
          p.fill(55, 65, 81); // gray
        }
